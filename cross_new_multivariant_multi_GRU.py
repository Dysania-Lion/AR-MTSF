...
for TH_path in TH_list:
    ...
    for p_l in pred_length:    
        ...
        for i_cluster in range(1, cluster_nums):
            ...
            def split_data(data, timestep, feature_size, partnumber):
                dataX = []  
                dataY = []  
            ...
            x_train, y_train, x_test, y_test, x_val = split_data(df, config.timestep, config.feature_size, 'Partnumber')
                        ...
                        for train_idx, test_idx in tscv.split(train_data):
                            ...
                            with torch.no_grad():
                                ...
                                for data_test in test_bar:
                                    ...
                                    test_loss += loss_function(y_test_pred, y_test_c.reshape(-1, Config.feature_size)).item()
            ...
            model = GRU(config.feature_size, best_params['hidden_size'], best_params['num_layers'],
                        config.output_size)
            model.cuda()
            ...
            for epoch in range(config.epochs):
                ...
                for data_train in train_bar:
                    ...
                    loss = loss_function(y_train_pred.reshape(-1, Config.feature_size),
                                         y_train.reshape(-1, Config.feature_size))
                    loss.backward()
                    optimizer.step()
                ...
                with torch.no_grad():
                    ...
                    for data_test in test_bar:
                        ...
                        test_loss += loss_function(y_test_pred,
                                                   y_test.reshape(-1, Config.feature_size)).item() 
