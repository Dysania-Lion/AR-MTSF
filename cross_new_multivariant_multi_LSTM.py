...
for p_l in pred_length:
    if p_l == 12:
        data_file = '多元data/all_data_weekly_selected_60_windowed_T_normalized-pred12.csv'
    elif p_l == 24:
        data_file = '多元data/all_data_weekly_selected_60_windowed_T_normalized-pred24.csv'
    else:
        print('路径有误')
        break
    ...
    for TH_path in TH_list:
        ...
        for i_cluster in range(1, cluster_nums):            
            output_csv_path = f'多元data/{TH_path}/pred{p_l}/多对多/matched_data{i_cluster}.csv'
            ...
            if p_l == 24:
                train, test = matched_data.loc[:, '2021-week1':'2022-week52'], matched_data.loc[:,
                                                                           '2023-week1':'2023-week25']
            elif p_l == 12:
                train, test = matched_data.loc[:, '2021-week1':'2022-week52'], matched_data.loc[:,
                                                                               '2023-week1':'2023-week13']
            ...
            def split_data(data, timestep, feature_size):
                dataX = []  
                dataY = []  
                windows_data_len = data.shape[1] - timestep
                ...
                for index in range(windows_data_len):
                    ...
                    y_data = df_transposed.iloc[index + timestep].values
                ...
                y_train = dataY[: train_size].reshape(-1, 1, feature_size)
                x_test = dataX[train_size:, :].reshape(-1, timestep, feature_size)
                y_test = dataY[train_size:].reshape(-1, 1, feature_size)
            ...
            x_train, y_train, x_test, y_test, x_val = split_data(df, config.timestep, config.feature_size)
            ...
            for hidden_size in param_grid['hidden_size']:
                for num_layers in param_grid['num_layers']:
                    for lr in param_grid['learning_rate']:
                        ...
                        model = LSTM(config.feature_size, hidden_size, num_layers, config.output_size)
                        model.cuda()
                        ...
                        for train_idx, test_idx in tscv.split(train_data):
                            ...
                            train_data_with_tensors = TensorDataset(subset_train_data[0],
                                                                    subset_train_data[1])
                            ...
                            train_data_with_tensors = TensorDataset(subset_test_data[0],
                                                                    subset_test_data[1])

                            test_loader = torch.utils.data.DataLoader(train_data_with_tensors,
                                                                      batch_size=config.batch_size,
                                                                      shuffle=False)
                            ...
                            for epoch in range(400):
                                ...
                                for data_train in train_bar:
                                    ...
                                    loss = loss_function(y_train_pred.reshape(-1, Config.feature_size),
                                                         y_train_c.reshape(-1, Config.feature_size))
                                    loss.backward()
                                    optimizer.step()
                            ...
                            with torch.no_grad():
                                ...
                                for data_test in test_bar:
                                    ...
                                    test_loss += loss_function(y_test_pred, y_test_c.reshape(-1,
                                                                                             Config.feature_size)).item()
            ...
            model = LSTM(config.feature_size, best_params['hidden_size'], best_params['num_layers'],
                        config.output_size)
            model.cuda()
            ...
            for epoch in range(config.epochs):
                ...
                for data_train in train_bar:
                    ...
                    loss = loss_function(y_train_pred.reshape(-1, Config.feature_size), y_train.reshape(-1, Config.feature_size))
                    loss.backward()
                    optimizer.step()
                    running_loss += loss.item()
                ...
                with torch.no_grad():
                    ...
                    for data_test in test_bar:
                        ...
                        test_loss += loss_function(y_test_pred, y_test.reshape(-1, Config.feature_size)).item()                
                avg_test_loss = test_loss / len(test_loader)                
            ...
            with open(f'Results/{TH_path}/pred{p_l}/多对多/loss_records{i_cluster}.csv', 'w', newline='') as file:
            ...
            part_row = predict          
            part_row.iloc[:] = predictions.T.reshape(Config.feature_size, predict_days)
            predict = part_row
            result = pd.concat([train, predict], axis=1)
            result.set_index(matched_data.index, inplace=True)
            
            result.to_csv(
                path_or_buf=f'Results/{TH_path}/pred{p_l}/多对多/multivariant_multi_result' + str(
